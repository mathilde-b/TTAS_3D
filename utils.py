...
def norm_soft_size_3d(a: Tensor, power:int) -> Tensor:
    b, c, w, h = a.shape
    amax = a.max(dim=1, keepdim=True)[0]+1e-10
    resp = (torch.div(a,amax))**power
    ress = einsum("bcwh->c", [resp]).type(torch.float32)
    ress_norm = ress/(torch.sum(ress,dim=0,keepdim=True)+1e-10)
    return ress_norm.unsqueeze(0)


def norm_soft_size(a: Tensor, power:int) -> Tensor:
    #print(a.shape,"in norm soft size")
    b, c, w, h = a.shape
    sl_sz = w*h
    amax = a.max(dim=1, keepdim=True)[0]+1e-10
...
def soft_centroid_3d(a: Tensor) -> Tensor:
    # returns the centroid in x, y, z
    b, c, w, h = a.shape

    bs, ws, hs = map_(lambda e: Tensor(e).to(a.device).type(torch.float32), np.mgrid[0:b,0:w, 0:h])
    assert bs.shape == ws.shape == hs.shape == (b, w, h)

    flotted = a.type(torch.float32)
    tot = einsum("bcwh->c", [a]).type(torch.float32) + 1e-10

    cb = einsum("bcwh,bwh->c", [flotted, bs]) / tot
    cw = einsum("bcwh,bwh->c", [flotted, ws]) / tot
    ch = einsum("bcwh,bwh->c", [flotted, hs]) / tot
    res = torch.stack([cb, cw, ch], dim=1).unsqueeze(0)
    assert res.shape == (1, c, 3)
...
def soft_dist_centroid_3d(a: Tensor) -> Tensor:
    b, k, w, h = a.shape
    img_shape = b,w,h
    # change grids shape ?
    ...
    sizes: Tensor = einsum("bk...->k", [flotted])
    assert sizes.dtype == torch.float32

    #centroids: list[Tensor] = [einsum("bkhw,hw->bk", flotted, grid) / (sizes + 1e-10)
    #                           for grid in tensor_grids]
    centroids = soft_centroid_3d(a).squeeze(0) #shape == (1, c, 3)
    #print(centroids.shape)
    centroids = [centroids[:,j] for j in range(0,3)]
    #print(centroids[0].shape)
    assert all(c.dtype == torch.float32 for c in centroids), [c.dtype for c in centroids]
    #print([c.shape for c in centroids])

    #assert all(c.shape == k for c in centroids)

    assert len(tensor_grids) == len(centroids), (len(tensor_grids), len(centroids))
    # Now the tricky part: different centroid for each batch and class:
    # g.shape: (bhw), c.shape: (k) -> d.shape: (kbhw)
    #print(tensor_grids[0].shape, tensor_grids[0].repeat(k, 1, 1, 1).shape)
    #print(centroids[0].shape)
    #print(centroids[0][:, None, None, None].shape)
    #print(centroids[0][:, None, None, None].repeat(1,*img_shape).shape)
    diffs: list[Tensor] = [(g.repeat(k, 1, 1, 1) - c[:,None, None, None].repeat(1,*img_shape))
                           for (g, c) in zip(tensor_grids, centroids)]
    diffs = [torch.transpose(d, 1, 0) for d in diffs]
    #print([d.shape for d in diffs])
    ...
    dist_centroid: list[Tensor] = [einsum("bkhw,bkhw->k", [flotted, d ** 2]) / (sizes + 1e-10)
                                   for d in diffs]

    # pprint(dist_centroid)
    res = torch.stack([dc.sqrt() for dc in dist_centroid], dim=1).unsqueeze(0)
    assert res.shape == (1, k, 3)
...
def class2one_hot(seg: Tensor, C: int) -> Tensor:
    ...
    #print(seg.shape,"in class2onehot")
    b, w, h = seg.shape  # type: Tuple[int, int, int]

    res = torch.stack([seg == c for c in range(C)], dim=1).type(torch.int32)
    assert res.shape == (b, C, w, h)
    assert one_hot(res)
    #print(res.shape,"in class2onehot")
    return res


def prt_uniq(seg: Tensor) -> Tensor:
...
def save_images_3d(segs: Tensor, names: Iterable[str], root: str, mode: str, iter: int, remap: True) -> None:
    #print(names)
    b, w, h = segs.shape  # Since we have the class numbers, we do not need a C axis
    i = 0
    for seg in segs:
        ...
        if remap:
            #assert sset(seg, list(range(2)))
            seg[seg == 1] = 255
        save_path = Path(root, f"iter{iter:03d}", mode, names[0]+'_'+str(i)).with_suffix(".png")
        save_path.parent.mkdir(parents=True, exist_ok=True)
        imsave(str(save_path), seg.astype('uint8'))
        i+=1
