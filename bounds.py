...
class PreciseBounds():
    ...
    def __call__(self, image: Tensor, target: Tensor, weak_target: Tensor, filename: str) -> Tensor:
        #print(target.shape,"in precise bounds")
        if self.namefun == "norm_soft_size" or self.namefun == "norm_soft_size_3d":
            if len(target.shape)==3:
                value: Tensor = self.__fn__(target[None, ...].type(torch.float32), self.power)[0].type(torch.float32)  # cwh and not bcwh
            else:
                value: Tensor = self.__fn__(target.type(torch.float32), self.power)[0].type(torch.float32)  # cwh and not bcwh
        else:
            if len(target.shape)==3:
                value: Tensor = self.__fn__(target[None, ...].type(torch.float32))[0].type(torch.float32)  # cwh and not bcwh
            else:
                value: Tensor = self.__fn__(target.type(torch.float32))[0].type(torch.float32)  # cwh and not bcwh
        margin: Tensor
        if self.mode == "percentage":
            margin = value * self.margin
        elif self.mode == "abs":
