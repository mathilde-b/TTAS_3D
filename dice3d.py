...
def sdice3d(all_grp, all_inter_card, all_card_gt, all_card_pred, all_pred, all_gt, all_pnames, metric_axis, pprint=False,
           do_hd=0, do_asd=0, best_epoch_val=0, savedir=0):
    unit = "mm"
    _, C = all_card_gt.shape
    unique_patient = torch.unique(all_grp)
    list(filter(lambda a: a != 0.0, unique_patient))
    assert (len(unique_patient) == 1)
    unique_patient = unique_patient[0].item()
    batch_dice = torch.zeros((1, C))
    batch_hd = torch.zeros((1, C))
    batch_asd = torch.zeros((1, C))
    if do_hd > 0 or do_asd > 0:
        all_pred = all_pred.cpu().numpy()
        all_gt = all_gt.cpu().numpy()
    # do DICE
    inter_card_p = torch.einsum("bc->c", [all_inter_card.reshape((-1, C))])
    card_gt_p = torch.einsum("bc->c", [all_card_gt.reshape((-1, C))])
    card_pred_p = torch.einsum("bc->c", [all_card_pred.reshape((-1, C))])
    dice_3d = (2 * inter_card_p + 1e-8) / ((card_pred_p + card_gt_p) + 1e-8)
    batch_dice[0, ...] = dice_3d
    if pprint:
        print(unique_patient, dice_3d.cpu(), file=open(savedir + '/' + str(p) + ".txt", "a"))
    ...
    if dice_3d_mean.mean() > 0:
        ...
        if do_hd > 0 or do_asd > 0:
            ...
            for j in range(0, C):
                label_pred_c = numpy.copy(all_pred)
                label_pred_c[label_pred_c != j] = 0
                label_pred_c[label_pred_c == j] = 1
                label_gt_c = numpy.copy(all_gt)
                label_gt_c[label_gt_c != j] = 0
                label_gt_c[label_gt_c == j] = 1
    ...
    if pprint:
        ...
        print('unit:', unit, 'asd_3d_mean', asd_3d, "asd_3d_sd", asd_3d_sd, "hd_3d_mean", hd_3d, "hd_3d_sd", hd_3d_sd,
              "dice 3d", dice_3d.item())
