...
from dice3d import sdice3d
from networks import weights_init
from dataloader import get_loaders
from utils import map_, save_dict_to_file, get_subj_nb
from utils import dice_coef, dice_batch, save_images, save_images_p, save_be_images, tqdm_, save_images_ent, save_images_3d
...
def do_epoch(args, mode: str, net: Any, device: Any, epc: int,
    ...
    #print(total_it_t,total_images_t,"total images")
    ...
    with warnings.catch_warnings():
        ...
        for j, target_data in tq_iter:
            ...
            if len(target_image.shape)==5:
                target_image = target_image.squeeze(0)
            if len(target_gt.shape)==5:
                target_gt = target_gt.squeeze(0)
            B = len(target_image)

            # initialize all
            all_dices: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_sizes: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_gt_sizes: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_sizes2: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_moments: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_moments_pred: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_moments_pred0: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_moments_gt: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_moments_gt0: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_inter_card: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_card_gt: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_card_pred: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_gt = []
            all_pred = []
            if args.do_hd or args.do_asd:
                all_gt: Tensor = torch.zeros((B, args.wh, args.wh), dtype=dtype)
                all_pred: Tensor = torch.zeros((B, args.wh, args.wh), dtype=dtype)
            loss_log: Tensor = torch.zeros((B), dtype=dtype, device=device)
            loss_cons: Tensor = torch.zeros((B), dtype=dtype, device=device)
            loss_se: Tensor = torch.zeros((B), dtype=dtype, device=device)
            loss_tot: Tensor = torch.zeros((B), dtype=dtype, device=device)
            posim_log: Tensor = torch.zeros((B), dtype=dtype, device=device)
            haussdorf_log: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_grp: Tensor = torch.zeros((B, C), dtype=dtype, device=device)
            all_pnames = np.zeros([B]).astype('U256')
            # Reset gradients
            ...
            for i, (loss_fn, label, w, bound) in enumerate(zip(loss_fns, labels, loss_weights, bounds)):
                ...
                if "EntKLProp" in eval(args.target_losses)[i][0]:
                    ...
                    loss_1, loss_size, loss_mom, est_prop = loss_fn(pred_probs, label, bound, epc)
                    if epc == 0 and args.adw:
                        first_loss = loss_size.detach()
                        keep_lambda = 1/first_loss
                    loss_kw.append(loss_1.detach())
                    if args.n_warmup > 0 and epc < n_warmup:
                        loss = loss_1
                    else:
                        loss = loss_1 + keep_lambda*(loss_size+loss_mom)
                        loss_kw.append(keep_lambda*(loss_size.detach()))
                        loss_kw.append(loss_mom.detach())
                else:
            ...
            if savedir and args.saveim and mode == "val":
                with warnings.catch_warnings():
                    ...
                    #print(predicted_class.shape,"save images")
                    save_images_3d(predicted_class, filenames_target, savedir, mode, epc, False)
                    save_images_3d(probs2class(target_gt), filenames_target, savedir, mode+"GT", epc, False)
                    if args.entmap:
    ...
    if args.dice_3d and (mode == 'val'):
        dice_3d_log, dice_3d_sd_log, asd_3d_log, asd_3d_sd_log, hd_3d_log, hd_3d_sd_log = sdice3d(all_grp,
                                                                                                 all_inter_card,
                                                                                                 all_card_gt,
                                                                                                 all_card_pred,
                                                                                                 all_pred, all_gt,
    ...
    losses_vec = [loss_se.mean().item(), loss_cons.mean().item(), loss_tot.mean().item(), soft_size_mean.mean(), hard_size_mean.mean(),
                  size_mean_pos.mean(), size_gt_mean.mean(), gt_size_mean_pos.mean()]
    #losses_vec = [loss_se.mean().item(), loss_cons.mean().item(), loss_tot.mean().item()]
    if args.ind_mom == 1:
        #mom_vec = [mom_est0, mom_est]
        mom_vec = [mom_est0, mom_est,mom_med_gt,mom_med_gt0]
    else:
        mom_vec = mom_est0
...
def run_subj(args, net, optimizer, device, loss_fns, loss_weights, scheduler, n_epoch, momfn):
    ...
    for i in range(n_epoch):
        ...
        if i == n_epoch - 1 and not args.oneslice:
            ...
            #last_folder_all = Path(savedir, "last_epoch")
            if last_folder.exists():
                rmtree(last_folder)
            if args.saveim:
                try:
                    copytree(Path(savedir, f"iter{i:03d}"), Path(last_folder))
                    #copytree(Path(savedir, f"iter{i:03d}"), Path(last_folder_all))
                except shutil.Error as err:
                    errors.extend(err.args[0])
        ...
        if not args.oneslice:
            df_t_tmp = pd.DataFrame({
                ...
                "val_soft_size_mean": [np.int(val_losses_vec[3])],
                "val_hard_size_mean": [np.int(val_losses_vec[4])],
                "val_gt_size_mean": [np.int(val_losses_vec[6])],
                "val_size_mean_pos": [np.int(val_losses_vec[5])],
                "val_gt_size_mean_pos": [np.int(val_losses_vec[7])],
