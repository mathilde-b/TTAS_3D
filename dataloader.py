...
from utils import simplex, sset, one_hot, pad_to, remap,prt_uniq
...
def get_loaders(args, data_folder: str, subfolders:str,
    ...
    nii_transform_3d = transforms.Compose([
        lambda nd: torch.tensor(nd, dtype=torch.float32),
        lambda nd: torch.transpose(nd, 2, 0),
        lambda nd: torch.transpose(nd, 1, 2),
        lambda nd: torch.unsqueeze(nd,1),
        #lambda nd: nd[:,0:384,0:384],
    ])
    ...
    nii_gt_transform_3d = transforms.Compose([
        lambda nd: torch.tensor(nd, dtype=torch.int64),
        lambda nd :torch.transpose(nd, 2, 0),
        lambda nd :torch.transpose(nd, 1, 2),
        #lambda nd : prt_uniq (nd),
        lambda nd: remap({0: 0, 2: 1, 1: 1}, nd),
        partial(class2one_hot, C=n_class),
        #lambda nd: nd[:,:,0:384,0:384],
        #itemgetter(0),
    ...
    if len(train_names)==0:
        train_names: List[str] = map_(lambda p: str(p.name), train_folders[0].glob(args.train_grp_regex+"*.nii.gz"))
    #train_names.sort()
    ...
    if len(val_names)==0:
        if "slice" in args.grp_regex:
            ...
            val_names: List[str] = map_(lambda p: str(p.name), val_folders[0].glob("*.nii"))
    if len(val_names)==0:
        val_names: List[str] = map_(lambda p: str(p.name), val_folders[0].glob("*.nii.gz"))
    if args.tta:
...
class SliceDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> List[Any]:
        ...
        elif path_name.suffix == ".nii" or path_name.suffix == ".gz":
            #print(files)
            try:
